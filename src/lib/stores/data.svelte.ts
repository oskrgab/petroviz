...
export async function loadWells(): Promise<void> {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to load wells";
    setError(message);
    throw error;
  }
}
...
export async function loadAvailableDateRange(): Promise<void> {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to load date range";
    setError(message);
    throw error;
  }
}
 ...
 */
export async function loadDailyProduction(
  forceWellIds?: number[],
): Promise<void> {
  try {
    const { wellIds: stateWellIds, dateRange } = getFilterParams();
    const wellIds =
      forceWellIds !== undefined
        ? forceWellIds.length > 0
          ? forceWellIds
          : undefined
        : stateWellIds;
 ...
 * @param forceWellIds - Optional: explicitly pass wellIds to override state
 */
export async function loadWellCumulatives(
  forceWellIds?: number[],
): Promise<void> {
  try {
    const { wellIds: stateWellIds, dateRange } = getFilterParams();
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to load well cumulatives";
    setError(message);
    throw error;
  }
}
...
export async function loadFieldTotals(forceWellIds?: number[]): Promise<void> {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to load field totals";
    setError(message);
    throw error;
  }
}
