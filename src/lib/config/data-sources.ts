...
function buildUrl(path: string): string {
	...
	const baseUrl = DATA_BASE_URL.endsWith('/') ? DATA_BASE_URL.slice(0, -1) : DATA_BASE_URL;
...
export function getConfigSummary(): {
	...
	if (env.PUBLIC_DAILY_PRODUCTION_PARQUET) configured.push('PUBLIC_DAILY_PRODUCTION_PARQUET');
	else usingDefaults.push('PUBLIC_DAILY_PRODUCTION_PARQUET');

	if (env.PUBLIC_MONTHLY_PRODUCTION_PARQUET) configured.push('PUBLIC_MONTHLY_PRODUCTION_PARQUET');
